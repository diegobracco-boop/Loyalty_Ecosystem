...
| `loyalty_ratio_acumulacion.json` | **Nuevo (10-sep, aún no consumido por el dashboard).** Ratio de acumulación Pasaporte D! (`SUM(points)/SUM(gb)`), solo `point_type='general'`, por mes × país × `grupo_pnl` (grupo del P&L de Control de Gestión, no el código crudo de Comarch — viene de la pestaña `regla producto` del Sheet Config, ver `load_regla_producto()`/`aggregate_ratio_acum()`). Falta el gráfico y correr la sync real. |
...
- **Ratio de acumulación Pasaporte D! (`loyalty_ratio_acumulacion.json`, 10-sep, commit `9bdb00d`) — pipeline listo, falta el gráfico.** `SUM(points)/SUM(gb)` por producto, **solo `point_type='general'`**. "Producto" = **grupo del P&L** (no el código crudo de Comarch) vía la pestaña nueva `regla producto` del Sheet Config — 19 filas que mezclan en la misma columna llave **nombres** de producto (`Vuelos`, `Hoteles`...) y **códigos** (`FLIGHT`, `HOTEL`...). Regla de match (Rosario): busca primero por nombre (`produto`, de `_ACUM_SQL` — **no** `produto_agrupado`, esa columna solo existe en `_REDEN_SQL`); si no matchea, cae a código (`product`, uppercase). Sin match → `"Sin clasificar"` + `[WARN]`. Fallback local `regla_producto.csv` (mismo patrón que `Diccionario.xlsx`/`breakage_esperado.csv` — `config_sheet_tab()` devuelve `None` en `--dry-run` a propósito). Cero SQL nuevo, cero riesgo de stages. **Pendiente**: correr la sync real (JSON no está en Drive todavía), decidir la forma del gráfico (heatmap producto×mes vs. barras por producto — sin cerrar), y que Rosario agregue `SAFEGUARD`/`Cfar` a la tabla (volumen despreciable, ~0,0007% del total, caen a "Sin clasificar" mientras tanto).
