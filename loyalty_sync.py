...
def enrich_acum_tier_usd(df_tier: pd.DataFrame, *acum_pre_dfs) -> pd.DataFrame:
    """Agrega `acum_usd_base` a df_tier (points por mes×país×tier) prorrateando el
    valor USD base de Pasaporte D! genuino (point_type='general', partner='DP') según
    la participación de puntos de cada tier dentro de su mes×país.

    Sin SQL nuevo (14-sep): _ACUM_TIER_SQL ya corre en un query aparte por el límite
    de 60 stages de Presto (ver comentario arriba de fetch_acum_tier) — duplicar ahí
    el cálculo de descuento_consumo_puntos_usd/pct_pagado_con_puntos de
    aggregate_acum() volvería a pisar ese límite. En cambio se reutiliza
    acum_usd_base sobre las mismas filas pre-agregadas que consume aggregate_acum()
    (_acum_cy_pre/_acum_ly_pre) y se reparte entre tiers según su % de puntos: un
    prorrateo simple, no un DRO por transacción (ese detalle no existe a nivel tier).

    Nota (auditoría 14-sep): el numerador (usd_tot) se filtra por partner='DP' +
    point_type='general', pero el denominador (points/points_tot, desde df_tier =
    _ACUM_TIER_SQL) solo filtra por point_type='general' — esa query no tiene columna
    partner. Misma asunción que ya usa aggregate_ratio_acum() (point_type='general' ==
    Pasaporte D! genuino == partner DP, confirmada por Rosario): si alguna vez
    apareciera un accrual 'general' de otro partner, el prorrateo no cerraría 1:1.
    """
    cols = list(df_tier.columns) + ["acum_usd_base"]
    if df_tier.empty:
        return df_tier.assign(acum_usd_base=pd.Series(dtype=float))[cols]

    frames = [d for d in acum_pre_dfs if d is not None and len(d)]
    if not frames:
        return df_tier.assign(acum_usd_base=0.0)[cols]
    d = pd.concat(frames, ignore_index=True)
    d = d[(d["point_type"].astype(str).str.lower() == _PASAPORTE_GENERAL_POINT_TYPE)
          & (d["partner"].astype(str) == "DP")].copy()
    if d.empty:
    ...
    for c in ("comision", "fee", "descuentos", "pct_pagado_con_puntos"):
        if c not in d.columns:
            d[c] = 0.0
    d["acum_usd_base"] = (
        (d["comision"].astype(float) + d["fee"].astype(float) + d["descuentos"].astype(float))
        * (1.0 - d["pct_pagado_con_puntos"].astype(float))
    ).abs()

    tot = (d.groupby(["processing_date", "country_code"], as_index=False)
             .agg(points_tot=("points", "sum"), usd_tot=("acum_usd_base", "sum")))

    out = df_tier.merge(tot, on=["processing_date", "country_code"], how="left")
    out["points_tot"] = out["points_tot"].fillna(0)
    out["usd_tot"] = out["usd_tot"].fillna(0)
    # Prorrateo por participación de puntos del tier dentro de su mes×país. Si el
    # denominador es 0 (mes/país sin detalle DP/general, o suma neta 0 por
    # cancelaciones que igualan el accrual) no se puede repartir -> 0, sin
    # división por cero.
    share = (out["points"] / out["points_tot"]).where(out["points_tot"] != 0, 0.0)
    out["acum_usd_base"] = (out["usd_tot"] * share).round(4)
    return out.drop(columns=["points_tot", "usd_tot"])[cols]
...
META_ACUM_TIER = {
    ...
    "fuente": "comarch_accumulation_report + comarch_cancellation_report, point_type='general' "
              ...
              "reconocimiento vigente en esa fecha. acum_usd_base (modo $) prorrateado desde "
              "_acum_cy_pre/_acum_ly_pre (point_type='general', partner='DP') según participación "
              "de puntos por mes×país×tier — ver enrich_acum_tier_usd().",
}
