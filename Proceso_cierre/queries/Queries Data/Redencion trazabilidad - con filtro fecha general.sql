    ...
    JOIN (
        ...
        GROUP BY base.transaction_code, t_trn.ext_country,
                 ...
                 CASE
                     ...
                     WHEN base.product_type = 'Universal'             THEN 'TICKET'
                     WHEN base.product_type = 'Alquileres'            THEN 'HOTEL'
                     WHEN base.product_type = 'SeaWork'               THEN 'TICKET'
                     WHEN base.product_type = 'Disney'                THEN 'TICKET'
                     WHEN base.product_type = 'Circuito'              THEN 'TOUR'
                     WHEN base.product_type = 'Tickets'               THEN 'TICKET'
                     WHEN base.product_type = 'Vuelos'                THEN 'FLIGHT'
...
-- ============================================================
-- tipopunto_raw — FIX APLICADO EN ar_ref (Rama 2)
-- ============================================================
tipopunto_raw AS (

    -- Rama 1: GR original + GA REFUND (sin cambios)
    SELECT
    ...
    -- Rama 2: contrapartida non-refundable — FIX en ar_ref
    SELECT
    ...
    INNER JOIN (
        ...
        WHERE ar.processing_date >= {{Desde}}
          ...
          -- Condición original: comarch no tiene REFUND positivo para estos tipos
          AND NOT EXISTS (
              SELECT 1
...
-- CTE intermedio: materializa window functions
proration AS (
    SELECT
        *,
...
-- CTE: calcula descuento_consumo_puntos_usd
final_calc AS (
    SELECT
        *,
        CAST(
            CASE
                ...
                THEN points_distribuidos * ratio_prd / 1.16
                ELSE points_distribuidos * ratio_prd
            END
        AS DECIMAL(18,2)) *-1 AS descuento_consumo_puntos_usd
    FROM proration
)
...
SELECT
    ...
    point_type,
    SUM(points_distribuidos)          AS points_distribuidos,
    business,
    country,
    produto_original,
    channel_condition,
    ...
    SUM(gb_basebi)                    AS gb_basebi,
    SUM(gb_basebi_2)                  AS gb_basebi_2,
